...
echo "⚠️  WARNING: This will DESTROY the SHARED infrastructure:"
...
manual_cleanup() {
    ...
    echo "🗑️  Deleting State Bucket ${STATE_BUCKET}..."
...
if [ $STATUS -ne 0 ]; then
    ...
    if [[ "$*" == *"--force"* ]]; then
        ...
        echo ""
        echo "💡 Tip: State might be corrupted. Use --force to perform a manual cleanup via AWS CLI."
        exit 1
    fi
fi

if [ -z "$STATE_BUCKET" ]; then
    echo "❌ Error: Could not resolve State Bucket. Is bootstrap deployed?"
    exit 1
fi
...
if [ ! -z "$ACTIVE_STATES" ]; then
    echo "❌ ABORTING: Active application states found:"
    echo "${ACTIVE_STATES}"
    echo ""
    if [[ "$*" == *"--force"* ]]; then
        echo "⚠️  FORCE flag detected. Proceeding despite active states..."
    else
        echo "Please run ./scripts/destroy-app.sh <env> for each of these environments first."
        echo "Or use ./scripts/destroy-bootstrap.sh <env> --force to bypass this check."
        exit 1
    fi
fi

echo "✅ No active apps found. Proceeding with destruction."
...
if [ -f "backend.tf" ]; then
    ...
    terraform init -migrate-state -force-copy > /dev/null || {
        echo "⚠️  State migration failed. Falling back to manual cleanup..."
        manual_cleanup
    }
else
...
terraform destroy -auto-approve || {
    echo "⚠️  Terraform destroy failed. Falling back to manual cleanup..."
    manual_cleanup
}
...
echo "✅ Shared Bootstrap resources destroyed."
